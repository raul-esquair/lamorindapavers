...
- ⚠️ **NEVER hand-append the brand to a `title`, anywhere.** The rule above is not blog-specific — it bit again. Until 2026-09-09 all three bespoke city pages set `title: "Paver Installation in <City>, CA | Lamorinda Pavers"` while the root layout *also* applied `template: "%s | Lamorinda Pavers"`, so production served **70-char titles with the brand twice** (`... | Lamorinda Pavers | Lamorinda Pavers`). Fixed in #43. Any page that does not set `titleAbsolute` gets the suffix automatically: write the authored title to **<=41 chars** so the templated result lands under Google's ~60, and never type the brand yourself. Verify with `curl -s <url> | grep -o '<title>[^<]*</title>'` — this class of bug is invisible in the source and obvious in the response.
- **`ServiceJsonLd`'s `provider` must stay an `@id` reference.** It used to spell out a `HomeAndConstructionBusiness` with name/phone/URL, which published a **second, detached** business entity on every page using it — splitting the entity graph rather than feeding it, the same failure the blog-author rule above prevents. `LocalBusinessJsonLd` renders the full node from the root layout on every page, so `provider: { "@id": BUSINESS_ID }` resolves. Fixed in #43; don't inline it back.
...
- **Orinda ranking work:** `plans/orinda-ranking/README.md` is the source of truth. Headline finding — `/orinda` ranks positions 3.8-12 for its money queries and earned **zero clicks in 90 days**, because for local commercial intent the map pack + ads + aggregators push organic #4 to roughly #12 visually. It is a visibility problem, not a ranking problem; more on-page content does not fix it. Items 1-3 shipped in #43. The unbuilt bet is a **Zone 0** section (CA ember-resistant-zone rules approved 2026-08-19, pavers a named compliant surface, MOFD inspects by named street) — informational queries with no map pack.
...
- **Config:** `blog.config.ts` (repo root) — brand strings, services, voice, geography, review recipients, and an optional **`author`** block (name/title/credential/experience). Loaded via `lib/blog-config.ts` (zod-validated). Committed because CI needs it. ⚠️ The repo is **public** (`raul-esquair/lamorindapavers`, verified 2026-09-13), not private as this line used to say — so everything in it is world-readable, including Steve's personal iCloud address in `reviewRecipients` (it is also hardcoded in `lib/reviews/recipients.ts`). Never commit anything here you wouldn't publish.
...
- ⚠️ **The nine templated `[city]` pages have NO JSON-LD and NO canonical at all** — `app/[city]/page.tsx` emits raw title+description only, while the three bespoke pages ship Breadcrumb + Service + FAQ schema and a canonical. This is a bigger hole than anything on the bespoke pages: `/berkeley` (319 impressions, pos 30.2), `/martinez` (268, pos 13.5) and `/dublin` (153, pos 13.8) were all high-impression pages in the 28 days to 2026-09-08. One edit to the shared template fixes all nine.
...
### Verifying visually in the browser tools — Lenis eats synthetic scrolls
`scroll_to` and wheel-tick scrolling both land in the wrong section: Lenis
intercepts wheel events globally and overrides native scroll, so the tools
overshoot or move the wrong way. Drive it with a computed delta instead —
dispatch a real `WheelEvent` carrying the exact remaining distance in a short
loop, letting Lenis's RAF settle between iterations, until the target is in
view. Also note `ScrollReveal` is scroll-scrubbed, so an element that has not
genuinely been scrolled past renders at opacity 0 and screenshots blank —
that is the animation working, not a bug.
